...
def enrich_filmes_with_favoritos(request, lista_filmes):
    if request.user.is_authenticated:
        try:
            lista_favoritos_padrao = ListaFavoritos.objects.get(
                usuario=request.user, padrao=True
            )
            favoritos_ids = set(
                lista_favoritos_padrao.filmes.values_list("id_tmdb", flat=True)
            )
...
def view_filmes_por_genero(request, id_genero):
    ...
    nome_genero_atual = next(
        (g.get("name") for g in generos_do_menu if g.get("id") == id_genero), "Gênero"
    )

    api_response = get_filmes_por_genero(
        id_genero, page=page_num, sort_by=sort_by_param
    )
    lista_de_filmes, pagina_atual, total_de_paginas = processar_resposta_api(
        api_response
    )
    ...
    context = {
        "titulo_secao": nome_genero_atual,
        "lista_filmes": lista_de_filmes,
        "pagina_atual": pagina_atual,
        "total_de_paginas": total_de_paginas,
        "numeros_de_pagina_para_template": gerar_numeros_paginacao(
...
def detalhe_filme(request, filme_id):
    ...
    listas_personalizadas_usuario = []
    if request.user.is_authenticated:
        is_favorito_atual = ListaFavoritos.objects.filter(
            usuario=request.user, padrao=True, filmes__id_tmdb=filme_id
        ).exists()
        listas_personalizadas_usuario = list(
            ListaFavoritos.objects.filter(usuario=request.user, padrao=False).values(
                "id", "nome"
    ...
    diretores = [
        m["name"]
        for m in detalhes.get("credits", {}).get("crew", [])
    ...
    context = {
        ...
        "base_profile_url": "https://image.tmdb.org/t/p/w185",
        "is_favorito_atual": is_favorito_atual,
        "listas_personalizadas_usuario": listas_personalizadas_usuario,
    }
...
def toggle_favorito(request):
    try:
        ...
        if created:
            ...
            if detalhes_filme:
                filme_obj.titulo = detalhes_filme.get("title", "Título não encontrado")
...
def add_filme_to_list_view(request):
    try:
        data = json.loads(request.body.decode("utf-8"))
        filme_obj, _ = Filme.objects.get_or_create(
            id_tmdb=int(data.get("id_tmdb")),
            defaults={
                "titulo": data.get("titulo"),
                "poster_path": data.get("poster_path", ""),
            },
        )
        lista_obj = ListaFavoritos.objects.get(
            id=data.get("id_lista"), usuario=request.user, padrao=False
        )
        ...
        if filme_obj in lista_obj.filmes.all():
            return JsonResponse(
                {
                    "status": "info",
                    "message": f"'{filme_obj.titulo}' já está na lista '{lista_obj.nome}'.",
                }
            )
        ...
        return JsonResponse(
            {
                "status": "ok",
                "message": f"'{filme_obj.titulo}' adicionado a '{lista_obj.nome}' com sucesso!",
            }
        )
        ...
        return JsonResponse(
            {
                "status": "error",
                "message": "Lista não encontrada ou não pertence a você.",
            },
            status=404,
        )
...
def remove_filme_from_list_view(request):
    try:
        ...
        lista_obj = ListaFavoritos.objects.get(
            id=data.get("id_lista"), usuario=request.user
        )
        ...
        return JsonResponse(
            {"status": "error", "message": "Lista ou filme não encontrado."}, status=404
        )
    except Exception:
        return JsonResponse(
            {"status": "error", "message": "Um erro inesperado ocorreu"}, status=500
        )